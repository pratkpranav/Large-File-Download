...
def download_chunk(host,tid,sock,chunk_queue,factor,startf,endf):
    ...
    try:
        inittime=time.time()
        checkf=0
        # print(host + ' ' + str(len(chunk_queue)))
        mt=chunk_queue.copy()
        while checkf < factor:
            ...
            for i in range(min(100,a-b)):
                ...
                # print(str((st,en)) + ' ' + host )
                # print(send_msg(host,st,en))
                sock.sendall(send_msg(host,st,en))
            ...
            #     resend.append((st,en))
            # print(resend)
            print('Thread id: ' + str(tid) + ' in time ' + str(exittime-inittime) + ' and Packets downloaded: ' + str(len(mt)-len(resend))  + ' From: ' + host)
...
def thread_op(tid,host,port,queue,st,en):
    ...
    download_chunk(host,tid,sock,queue,1,st,en)
...
def main():
    ...
    for i in range(len(data_read)):
        ...
        full_chunk=6488666
        chunk_size=10000
        num_threads=int(data_read[i][2])
        hostsiteone=data_read[i][0].split('/',1)
        hostsitetwo=data_read[i][1].split('/',1)
        # print(hostsiteone)
        # print(hostsitetwo)
        # print(st)
        ...
        hostone = hostsiteone[0]
        hosttwo = hostsitetwo[0]
        port = 80
        for i in range(num_threads):
            ...
            if i < int(num_threads/2):
                t=threading.Thread(target=thread_op,args=(i,hostone,port,queue_arr[i],st2,en1))
                thread_arr.append(t)
            else:
                t=threading.Thread(target=thread_op,args=(i,hosttwo,port,queue_arr[i],st2,en1))
                thread_arr.append(t)
